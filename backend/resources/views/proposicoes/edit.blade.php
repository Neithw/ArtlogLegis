<x-app-layout>
    ...
    <div class="py-4">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('proposicoes.update', $proposicao) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="overflow-hidden rounded-xl bg-white shadow">
                    <div class="border-b border-gray-200 px-6 py-5">
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            Altere os dados da proposição.
                        </p>
                    </div>

                    <div class="grid gap-6 p-6 md:grid-cols-2" x-data="{
                        legislaturaId: @js((string) old('legislatura_id', $proposicao->legislatura_id)),
                    
                        autorMandatoId: @js((string) old('autor_mandato_id', $proposicao->autor_mandato_id)),
    ...
    $mandatos
        ...
        ->values(),
                        ...
                        <div>
                            ...
                            <select id="legislatura_id" name="legislatura_id" x-model="legislaturaId"
                                ...
                                @foreach ($legislaturas as $legislatura)
                                    <option value="{{ $legislatura->id }}" @selected(old('legislatura_id', $proposicao->legislatura_id) == $legislatura->id)>
                                        {{ $legislatura->numero }}ª
                                        Legislatura{{ $legislatura->trashed() ? ' (arquivada)' : '' }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="autor_mandato_id" name="autor_mandato_id" x-model="autorMandatoId"
                                ...
                                <template x-for="mandato in mandatosFiltrados" :key="mandato.id">
                                    <option :value="String(mandato.id)"
                                        :selected="String(mandato.id) === String(autorMandatoId)"
                                        x-text="mandato.nome + (mandato.arquivado ? ' (arquivado)' : '')"></option>
                                </template>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="tipo_proposicao_id" name="tipo_proposicao_id"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:bg-gray-100 disabled:text-gray-500"
                                required>
                                <option value="">
                                    Selecione um Tipo
                                ...
                                @foreach ($tiposProposicao as $tipo)
                                    <option value="{{ $tipo->id }}" @selected(old('tipo_proposicao_id', $proposicao->tipo_proposicao_id) == $tipo->id)>
                                        {{ $tipo->nome }}{{ $tipo->trashed() ? ' (arquivado)' : '' }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-input id="ementa" name="ementa" type="text" class="mt-1 block w-full"
                                :value="old('ementa', $proposicao->ementa)" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-input id="assunto" name="assunto" type="text" class="mt-1 block w-full"
                                :value="old('assunto', $proposicao->assunto)" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="texto_integral" name="texto_integral" rows="12"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('texto_integral', $proposicao->texto_integral) }}</textarea>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-input id="area_tematica" name="area_tematica" type="text" class="mt-1 block w-full"
                                :value="old('area_tematica', $proposicao->area_tematica)" />
                        ...
                        <div class="md:col-span-2" x-data="{
                            ...
                            palavrasChave: @js(
    collect(old('palavras_chave', $proposicao->palavras_chave ?? []))
